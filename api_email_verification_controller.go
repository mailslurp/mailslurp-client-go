...
/*
GetEmailIntelligence Get intelligence score and breakdown for email/domain targets. Per unit billing for non-cached evaluations.
...
*/
func (a *EmailVerificationControllerApiService) GetEmailIntelligence(ctx _context.Context, emailIntelligenceOptions EmailIntelligenceOptions) (EmailIntelligenceListResult, *_nethttp.Response, error) {
	var (
		...
		localVarFileName     string
		localVarFileBytes    []byte
		localVarReturnValue  EmailIntelligenceListResult
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/email-verification/intelligence"
	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := _neturl.Values{}
	localVarFormParams := _neturl.Values{}
	...
	// body params
	localVarPostBody = &emailIntelligenceOptions
	if ctx != nil {
		// API Key Authentication
		if auth, ok := ctx.Value(ContextAPIKey).(APIKey); ok {
			var key string
