...
/*
UploadAttachment Upload an attachment for sending using base64 file encoding
...
/*
UploadAttachmentBytes Upload an attachment for sending using file byte stream input octet stream
When sending emails with attachments first upload each attachment with this endpoint. Record the returned attachment IDs. Then use these attachment IDs in the SendEmailOptions when sending an email. This means that attachments can easily be reused.
 * @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
...
func (a *AttachmentControllerApiService) UploadAttachmentBytes(ctx _context.Context, localVarOptionals *UploadAttachmentBytesOpts) ([]string, *_nethttp.Response, error) {
	...
	if localVarOptionals != nil && localVarOptionals.Filename.IsSet() {
		localVarPostBody = localVarOptionals.Filename.Value()
	}
